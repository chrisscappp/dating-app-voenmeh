...
@app.delete("/deleteAccount/{user_id}")
...
def change_password(user_id: str, user: DataReset):
    if user.oldPassword == user.newPassword:
        raise HTTPException(status_code=403, detail="Passwords should be different")
    try:
        auth.sign_in_with_email_and_password(user.email, user.oldPassword)
    except HTTPError as e:
        error_json = e.args[1]
        error = json.loads(error_json)['error']['message']
        if error == "INVALID_EMAIL":
